To show use of multiResourceItem reader with a JsonItemReader. 
 Modified spring batch sample to read .json files in the resource directory.
...
If you use Maven, visit the https://start.spring.io/#!type=maven-project&language=java&platformVersion=2.4.3.RELEASE&packaging=jar&jvmVersion=1.8&groupId=com.example&artifactId=batch-processing&name=batch-processing&description=Demo%20project%20for%20Spring%20Boot&packageName=com.example.batch-processing&dependencies=batch,hsql[Spring Initializr] to generate a new project with the required dependencies (Spring Batch and HyperSQL Database).
